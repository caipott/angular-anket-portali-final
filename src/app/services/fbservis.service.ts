    ...
    return this.aktifUye.pipe(
      ...
      concatMap((user) =>
        addDoc(ref, {
          ...
          tamam: gorev.tamam,
          uid: user?.uid,
          soruBir: gorev.soruBir,
          soruIki: gorev.soruIki,
          soruUc: gorev.soruUc,
          soruDort: gorev.soruDort,
          soruBes: gorev.soruBes,
          soruAlti: gorev.soruAlti,
        })
      ),
      map((ref) => ref.id)
    );
  ...
  CevapListele() {
    ...
    return this.aktifUye.pipe(
      concatMap((user) => {
        ...
        return collectionData(myQuery, { idField: 'uid' }) as Observable<Cevap[]>;
      })
    );
  }


  CevapEkle(cevap: Cevap, gorev: Gorev) {
    var ref = collection(this.fs, "Cevaplar");
    return this.aktifUye.pipe(
      take(1),
      concatMap((user) =>
        addDoc(ref, {
          uid: user?.uid,
          soruBir: cevap.soruBir,
          soruIki: cevap.soruIki,
          soruUc: cevap.soruUc,
          soruDort: cevap.soruDort,
          soruBes: cevap.soruBes,
          soruAlti: cevap.soruAlti,
